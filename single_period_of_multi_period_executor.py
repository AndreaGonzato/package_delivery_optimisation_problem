...
def solve_period(stores, vehicles, customers):
    ...
    x_l_i_j = model.addVars([(l, i, j) for i in range(I_LF) for j in range(J_LF) for l in range(L)],
                            vtype=gb.GRB.BINARY)
    ...
    y_l_i_j = model.addVars([(l, i, j,) for i in range(I_LF) for j in range(J_LF) for l in range(L)],
