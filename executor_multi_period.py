...
def solve_period(stores, vehicles, customers):
    ...
    x_l_i_j = model.addVars([(l, i, j) for i in range(I_LF) for j in range(J_PF) for l in range(L)],
                            vtype=gb.GRB.BINARY)
    ...
    y_l_i_j = model.addVars([(l, i, j,) for i in range(I_LF) for j in range(J_PF) for l in range(L)],
    ...
    w_c_k = model.addVars([(c, k) for k in range(len(OC_unique)) for c in range(len(Sk))], vtype=gb.GRB.BINARY, name="w_c_k")

    # define constraints Customers’ service

    # constraint eq. 2
    for c in range(len(C_D)):
        model.addConstr(z_c[c] + gb.quicksum(w_c_k[s, k] for s in range(len(Sk)) for k in range(len(OC_unique))
                                             if C_D[c] == filter_cd_sk[s]) + gb.quicksum(
            z_c_l[l, c] for l in range(L)) == 1)
    # constraint eq. 3
